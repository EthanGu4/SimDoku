...
- [x] **Phase 2 — Algorithm plug-in expansion.** 2–4 more classical solvers (e.g. constraint propagation, Dancing Links, simulated annealing), one at a time. Later joined by `x_wing_swordfish` (`app/solvers/fish_patterns.py`), which layers the X-Wing and Swordfish candidate eliminations on top of constraint propagation. Its single-finding order deliberately mirrors `constraint_propagation` exactly, so the two produce identical traces on puzzles where no fish pattern fires and any difference is attributable to the eliminations alone (a test enforces this). The patterns fire on roughly 1 in 7 bank puzzles and cut total steps ~5%.
