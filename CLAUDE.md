...
- [x] **Phase 3 — Benchmark harness & "race mode".** Reworked from its original single-puzzle/history-table form into a live, genuinely concurrent race: every algorithm runs against the same 100 puzzles per difficulty (easy/medium/hard), pulled from a real graded dataset (`grantm/sudoku-exchange-puzzle-bank`, public domain) and committed as a static fixture — see `backend/app/core/data/race_puzzles.json` and `scripts/import_race_puzzles.py`. `POST /race/start` kicks off one subprocess per algorithm and returns immediately; the frontend polls `GET /race/{race_id}/progress` (~300ms cadence) so each algorithm's panel shows genuinely divergent live progress — a fast algorithm visibly pulls ahead — instead of a synchronized, faked-pace reveal. Each subprocess is wall-clock capped (`app/core/race_runner.py`) so a struggling algorithm (simulated annealing especially) can never run forever; this is a deliberate, documented exception to the app's usual eager-compute/no-streaming rule, scoped to just this endpoint. No run history/persistence anymore — race mode is stateless per race.
...
- [ ] **Phase 6 — ML: algorithm picker / meta-solver.** A pseudo-solver that delegates to a registered algorithm. Since Phase 3 no longer persists run history, this would need to either train on-demand from the race dataset/endpoints or reintroduce some form of persisted results — not yet decided.
